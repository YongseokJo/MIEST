...
class MIST():
    ...
    def train(self,which_machine="vib+cls",fname="test",beta=1, gamma=1e-2, 
              learning_rate=1e-3, max_lr=1e-9,
        ...
        if which_machine == 'xgboost':
            ...
            self.trainer = Trainer(self.vib, self.cls, self.train_loader, which_machine,
                              self.val_dataset,fname,self.device, num_sim=self.num_sim)
            if which_machine == 'vib_cnn':
                __train__ = self.trainer.run_cnn
            else:
                __train__ = self.trainer.run
            res     = __train__(learning_rate, max_lr, epochs, beta, gamma,
                            patience, tol,
    ...
    def get_valid_loss(self):
        self.vib.eval()
        val_loss = 0.
        for N, X_val, y_val in enumerate(valid_loader):
            with torch.no_grad():
                y_param        = y_val[:,:2].cpu().detach().numpy()
                y_pred, _      = self.trainer.vib(X_val.to(self.device))
                y_pred         = y_pred.detach().cpu().numpy()
                diff           = (y_param-y_pred)**2
    ...
    def make_train_set(self,imaginary=False):
        ...
        N_sample = y_params.shape[0]
        N_output = y_params.shape[1] 
        y        = torch.zeros((N_sample, 
                                N_output + self.num_sim))
        y[:,:N_output] = y_params
        if self.projection:
            N = 3000
        elif self.average:
    ...
    def load_optuna_models(self, storage, study_name, metric=None, suffix='',
        ...
        if self.which_machine == 'vib':
            ...
            self.vib = VIB_CNN(hidden=params['hidden'], dr=params['dr'],
                               channels=1,
                               z_dim=params['z_dim'])
            self.cls = None
        elif which_machine == 'vib+cls':
        ...
        if print_loss and which_machine in self.mach_vib:
            print("num_trial={}, params={} ".format(num_trial, params))
            pass
