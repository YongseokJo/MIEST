...
for field in ['Mtot', 'T', 'Mgas','Mstar','HI','ne','Vcdm','Z']:
    if TNG:
        ...
        if projection:
            prefix += 'p'
        np.save("../data/wph_{}IllustrisTNG_{}_for_vib_total{}".format(prefix,field,suffix),coef)
    ...
    if SIMBA:
        ...
        if projection:
            prefix += 'p'
        np.save("../data/wph_{}SIMBA_{}_for_vib_total{}".format(prefix,field,suffix),coef)
